...
#-------------------------------------------------------------------------------------------------------
# diagonal matrix with respect to given patch Dirichlet !!!!
#--------------------------------------------------------------------------------------------------------
def assemble_matrix_DirichletSpacediagnitsche(
    ne1: 'int', ne2: 'int',
    p1: 'int', p2: 'int', p3: 'int', p4: 'int',
    spans_1: 'int[:]', spans_2: 'int[:]',spans_3: 'int[:,:]', spans_4: 'int[:,:]',
    basis_1: 'float[:,:,:,:]', basis_2: 'float[:,:,:,:]',basis_3: 'float[:,:,:,:]', basis_4: 'float[:,:,:,:]',
    ...
    omega_1: 'float[:]', omega_2: 'float[:]',
    interfaces: 'int[:]', Kappa: 'float',
    normalS: 'float', coDir:'int', matrix: 'float[:,:,:,:]'
):
    #..assemble  solution times noraml(test fuction)
    from numpy import zeros
    from numpy import sqrt
                ...
                for il_2 in range(0, p2+1):
                    for jl_2 in range(0, p2+1):
                        ...
                        for g2 in range(0, k2):
                            ...
                            wvol    = weights_2[ie2, g2]
                            # ...
                            v      +=  normalS * ((1-coDir)*comp_1 * bj_0 + bi_0*comp_2*coDir) * wvol + Kappa * bi_0 * bj_0 * wvol * sqrt(F_1y[g2]**2 + F_2y[g2]**2)
                            vip    +=  normalS * ((1-coDir)*comp_3 * bj_0) * wvol
                            vjp    +=  normalS * (bi_0*comp_4*coDir) * wvol
                ...
                for il_2 in range(0, p2+1):
                    for jl_2 in range(0, p2+1):
                        ...
                        for g2 in range(0, k2):
                            ...
                            wvol  = weights_2[ie2, g2]
                            # ... - 0.5*u1*v1_n - 0.5*u1_n*v1
                            v    += normalS * ((1-coDir)*bj_0 * comp_1 + comp_2 * bi_0*coDir)  * wvol + Kappa * bi_0 * bj_0 * wvol * sqrt(F_1y[g2]**2 + F_2y[g2]**2)
                            vpi  += normalS * ((1-coDir)*bj_0 * comp_3)  * wvol 
                            vpj  += normalS * ( comp_4 * bi_0*coDir)  * wvol
                ...
                for il_1 in range(0, p1+1):
                    for jl_1 in range(0, p1+1):
                        ...
                        for g1 in range(0, k1):
                            ...
                            wvol            = weights_1[ie1, g1]
                            # ...
                            v              +=  normalS * ((1-coDir)*comp_1 * bj_0 + bi_0*comp_2*coDir) * wvol + Kappa * bi_0 * bj_0 * wvol * sqrt(F1_1x[g1]**2 + F1_2x[g1]**2)
                            vip            +=  normalS * ((1-coDir)*comp_3 * bj_0) * wvol
                            vjp            +=  normalS * (bi_0*comp_4*coDir) * wvol
