...
#==============================================================================
#---2 : B-splines and thier corresponding spanes in adapted mesh
def assemble_nurbsbasis_spans_in_adquadrature_1DL2mapGmesh(ne1:'int', p1:'int', p2:'int', spans_1:'int[:,:]', basis_1:'float64[:,:,:,:]', weights_1:'float64[:,:]', knots_1:'float64[:]', omega:'float64[:]', vector_u:'float64[:]',  spans_ad:'int[:,:]', basis_ad:'float64[:,:,:,:]', nders:'int'):
...
#---2 : B-splines and thier corresponding spanes in adapted mesh in refined integral
def assemble_nurbsbasis_spans_in_adquadrature_L2mapGmesh(ne1:'int', ne2:'int', p1:'int', p2:'int', p3:'int', p4:'int', spans_1:'int[:,:]', spans_2:'int[:,:]', basis_1:'float64[:,:,:,:]', basis_2:'float64[:,:,:,:]', weights_1:'float64[:,:]', weights_2:'float64[:,:]', knots_1:'float64[:]', knots_2:'float64[:]', omega_1:'float64[:]', omega_2:'float64[:]', vector_u1:'float64[:,:]', vector_u2:'float64[:,:]', spans_ad1:'int[:,:,:,:]', spans_ad2:'int[:,:,:,:]', basis_ad1:'float64[:,:,:,:,:,:]', basis_ad2:'float64[:,:,:,:,:,:]', nders:'int'):
                   ...
                   for g2 in range(0, k2):
                      for g3 in range(0, k3):
                            ...
                            for i_ders in range(1,nders+1):
                                ...
                                for j_ders in range(0,i_ders):
                                    sum_basistmp = sum(ders3[i_ders-j_ders,:])/sum_basisx
                                    num = 1
                                    den = 1
                                    m_jders = j_ders + 1
                                    for t in range(m_jders):
