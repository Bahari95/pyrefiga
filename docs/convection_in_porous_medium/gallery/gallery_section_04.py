__all__ = ['assemble_matrix_diff_ex01',
           'assemble_vector_diff_ex01']

def assemble_matrix_diff_ex01(ne1:'int', ne2:'int', p1:'int', p2:'int', p3:'int', p4:'int',
                            ...
                            vector_st:'float64[:,:]', vector_m1:'float64[:,:]', vector_m2:'float64[:,:]', 
                            dt:float, Le:float, ratioPH:float, matrix:'float64[:,:,:,:]'):
    ...
    # ...
    lcoeffs_st = zeros((p1+1,p2+1))
    lcoeffs_m1 = zeros((p3+1,p4+1))
    lcoeffs_m2 = zeros((p3+1,p4+1))

    # ...
    arr_dxst   = zeros((k1,k2))
    arr_dyst   = zeros((k1,k2))
    #...
    ...
    for ie1 in range(0, ne1):
        ...
        for ie2 in range(0, ne2):
            ...
            lcoeffs_st[ : , : ] = vector_st[ i_span_1 : i_span_1+p1+1, i_span_2 : i_span_2+p2+1]
                        ...
                        for jl_2 in range(0, p2+1):
                            ...
                            for g1 in range(0, k1):
                                for g2 in range(0, k2):
                                    ...
                                    v    += bi_0 * bj_0 * wvol* J_mat[g1, g2] + advec * wvol  + dt*ratioPH/Le * (bi_x * bj_x + bi_y * bj_y ) * wvol / J_mat[g1,g2]

                            matrix[p1+i1, p2+i2, p1+j1-i1, p2+j2-i2]  += v
    # ...
#==============================================================================
#---2 : rhs
def assemble_vector_diff_ex01(ne1:'int', ne2:'int', p1:'int', p2:'int', p3:'int', p4:'int',
                            spans_1:'int[:]', spans_2:'int[:]',spans_3:'int[:,:]', spans_4:'int[:,:]',
                            basis_1:'float64[:,:,:,:]', basis_2:'float64[:,:,:,:]',  basis_3:'float64[:,:,:,:]', basis_4:'float64[:,:,:,:]',
                            weights_1:'float64[:,:]', weights_2:'float64[:,:]', points_1:'float64[:,:]', points_2:'float64[:,:]', 
                            vector_m1:'float64[:,:]', vector_m2:'float64[:,:]', vector_d:'float64[:,:]', vector_t:'float64[:,:]', vector_c:'float64[:,:]', 
                            Ra:'float', Nr:'float', rhs:'float64[:,:]'):
    ...
    lcoeffs_c   = zeros((p1+1,p2+1))
    lcoeffs_tm  = zeros((p1+1,p2+1))
    #..
    ...
    for ie1 in range(0, ne1):
        ...
        for ie2 in range(0, ne2):
            ...
            lcoeffs_tm[ : , : ] = vector_t[ i_span_1 : i_span_1+p1+1, i_span_2 : i_span_2+p2+1]
            lcoeffs_c[ : , : ]  = vector_c[ i_span_1 : i_span_1+p1+1, i_span_2 : i_span_2+p2+1]
            for g1 in range(0, k1):
                for g2 in range(0, k2):
                    ...
                    for il_1 in range(0, p1+1):
                          for il_2 in range(0, p2+1):
                              ...
                              coeff_tm =  lcoeffs_tm[il_1,il_2]
                              tmx     +=  coeff_tm * bj_x                              
                              tmy     +=  coeff_tm * bj_y                              
                              coeff_c  =  lcoeffs_c[il_1,il_2]
                              cx      +=  coeff_c * bj_x
                              cy      +=  coeff_c * bj_y
                    # derivatives of psi
                    psi_x = x #pi* cos(pi*x)*sin(pi*y)
                    psi_y = y #pi* sin(pi*x)*cos(pi*y)
                    # magnetic field B = (-∂y psi, ∂x psi)
                    Bx = -psi_y          # = y
                    By = psi_x           # = -0.5*x*(x^2-1)
                    # norm squared
                    normB2 = Bx**2 + By**2
                    ...
                    c_per      = 1e0   #perpendicular coefficient
                    c_par      = 0e+1 #parallel coefficient
                    K11    =  c_per + (c_par - c_per) * bb11
                    K12    = (c_par - c_per) * bb12
                    K22    = c_per + (c_par - c_per) * bb22
...
#==============================================================================
#---2 : rhs
def assemble_vector_mass_ex01(ne1:'int', ne2:'int', p1:'int', p2:'int', p3:'int', p4:'int',
                            spans_1:'int[:]', spans_2:'int[:]',spans_3:'int[:,:]', spans_4:'int[:,:]',
                            basis_1:'float64[:,:,:,:]', basis_2:'float64[:,:,:,:]',  basis_3:'float64[:,:,:,:]', basis_4:'float64[:,:,:,:]',
                            weights_1:'float64[:,:]', weights_2:'float64[:,:]', points_1:'float64[:,:]', points_2:'float64[:,:]', 
                            vector_m1:'float64[:,:]', vector_m2:'float64[:,:]', vector_s:'float64[:,:]', rhs:'float64[:,:]'):
    from numpy import zeros
    ...
    for ie1 in range(0, ne1):
        ...
        for ie2 in range(0, ne2):
            ...
            lcoeffs_s[ : , : ]  = vector_s[ i_span_1 : i_span_1+p1+1, i_span_2 : i_span_2+p2+1]
            for g1 in range(0, k1):
                for g2 in range(0, k2):
                    ...
                    for il_1 in range(0, p1+1):
                          for il_2 in range(0, p2+1):
                              ...
                              coeff_s =  lcoeffs_s[il_1,il_2]
                              u_s     +=  coeff_s * bj_0
                    J_mat = abs(F1x*F2y-F1y*F2x)
                    lvalues_u[g1,g2]   = u_s * J_mat
            for il_1 in range(0, p1+1):
                for il_2 in range(0, p2+1):
                    i1 = i_span_1 - p1 + il_1
                    i2 = i_span_2 - p2 + il_2
        ...
        if interface_nb == 1:
            ...
            for ie2 in range(0, ne2):
                ...
                for g2 in range(0, k2):
                    ...
                    c_per             = 1e0   #perpendicular coefficient
                    c_par             = 0e+1 #parallel coefficient                    
                    arr_K11[g2]    =  c_per + (c_par - c_per) * bb11
                    arr_K12[g2]    = (c_par - c_per) * bb12
                    arr_K22[g2]    = c_per + (c_par - c_per) * bb22
                    # ... compute the normal derivative
                    F_1x[g2] = F1x
                    F_2x[g2] = F2x
                    F_1y[g2] = F1y
                ...
                for il_2 in range(0, p2+1):
                    for jl_2 in range(0, p2+1):
                        ...
                        for g2 in range(0, k2):
                            ...
                            # ...
                            G_1 = arr_K11[g2]*( F_2y[g2]*bi_x - F_2x[g2]*bi_y)/J_mat2[g2] + arr_K12[g2]*(-F_1y[g2]*bi_x + F_1x[g2]*bi_y)/J_mat2[g2]
                            G_2 = arr_K12[g2]*( F_2y[g2]*bi_x - F_2x[g2]*bi_y)/J_mat2[g2] + arr_K22[g2]*(-F_1y[g2]*bi_x + F_1x[g2]*bi_y)/J_mat2[g2]
                            comp_1          = +1 * G_1 * F_2y[g2] #/sqrt(F1y**2+ F2y**2)
                            comp_1         += -1 * G_2 * F_1y[g2] #/sqrt(F1y**2+ F2y**2)
                            # ...
                            G_1 = arr_K11[g2]*(F_2y[g2]*bj_x - F_2x[g2]*bj_y)/J_mat2[g2] + arr_K12[g2]*(-F_1y[g2]*bj_x + F_1x[g2]*bj_y)/J_mat2[g2]
                            G_2 = arr_K12[g2]*(F_2y[g2]*bj_x - F_2x[g2]*bj_y)/J_mat2[g2] + arr_K22[g2]*(-F_1y[g2]*bj_x + F_1x[g2]*bj_y)/J_mat2[g2]
                            comp_2          = +1 * G_1 * F_2y[g2] #/sqrt(F1y**2+ F2y**2)
                            comp_2         += -1 * G_2 * F_1y[g2] #/sqrt(F1y**2+ F2y**2)
                            # ...
                            G_1 = arr_K11[g2]*( F_2y[g2]*bi_px - F_2x[g2]*bi_y)/J_mat2[g2] + arr_K12[g2]*(-F_1y[g2]*bi_px + F_1x[g2]*bi_y)/J_mat2[g2]
                            G_2 = arr_K12[g2]*( F_2y[g2]*bi_px - F_2x[g2]*bi_y)/J_mat2[g2] + arr_K22[g2]*(-F_1y[g2]*bi_px + F_1x[g2]*bi_y)/J_mat2[g2]
                            comp_3          = +1 * G_1 * F_2y[g2] #/sqrt(F1y**2+ F2y**2)
                            comp_3         += -1 * G_2 * F_1y[g2] #/sqrt(F1y**2+ F2y**2)
                            # ...
                            G_1 = arr_K11[g2]*( F_2y[g2]*bj_px - F_2x[g2]*bj_y)/J_mat2[g2] + arr_K12[g2]*(-F_1y[g2]*bj_px + F_1x[g2]*bj_y)/J_mat2[g2]
                            G_2 = arr_K12[g2]*( F_2y[g2]*bj_px - F_2x[g2]*bj_y)/J_mat2[g2] + arr_K22[g2]*(-F_1y[g2]*bj_px + F_1x[g2]*bj_y)/J_mat2[g2]
                            comp_4          = +1 * G_1 * F_2y[g2] #/sqrt(F1y**2+ F2y**2)
                            comp_4         += -1 * G_2 * F_1y[g2] #/sqrt(F1y**2+ F2y**2)
                            # ...
        ...
        if interface_nb == 3:
            ...
            for ie1 in range(0, ne1):
                ...
                for g1 in range(0, k1):
                    ...
                    c_per             = 1e0   #perpendicular coefficient
                    c_par             = 0e+1 #parallel coefficient
                    arr1_K11[g1]    =  c_per + (c_par - c_per) * bb11
                    arr1_K12[g1]    = (c_par - c_per) * bb12
                    arr1_K22[g1]    = c_per + (c_par - c_per) * bb22
                    # ... compute the normal derivative
                    F1_1x[g1]  = F1x
                    F1_2x[g1]  = F2x
                    F1_1y[g1]  = F1y
                ...
                for il_1 in range(0, p1+1):
                    for jl_1 in range(0, p1+1):
                        ...
                        for g1 in range(0, k1):
                            ...
                            # ...
                            G_1 = arr1_K11[g1]*( F1_2y[g1]*bi_x - F1_2x[g1]*bi_y)/J_mat1[g1] + arr1_K12[g1]*(-F1_1y[g1]*bi_x + F1_1x[g1]*bi_y)/J_mat1[g1]
                            G_2 = arr1_K12[g1]*( F1_2y[g1]*bi_x - F1_2x[g1]*bi_y)/J_mat1[g1] + arr1_K22[g1]*(-F1_1y[g1]*bi_x + F1_1x[g1]*bi_y)/J_mat1[g1]
                            comp_1          = -1 * G_1 * F1_2x[g1] #/sqrt(F1y**2+ F2y**2)
                            comp_1         += +1 * G_2 * F1_1x[g1] #/sqrt(F1y**2+ F2y**2)
                            # ...
                            G_1 = arr1_K11[g1]*( F1_2y[g1]*bj_x - F1_2x[g1]*bj_y)/J_mat1[g1] + arr1_K12[g1]*(-F1_1y[g1]*bj_x + F1_1x[g1]*bj_y)/J_mat1[g1]
                            G_2 = arr1_K12[g1]*( F1_2y[g1]*bj_x - F1_2x[g1]*bj_y)/J_mat1[g1] + arr1_K22[g1]*(-F1_1y[g1]*bj_x + F1_1x[g1]*bj_y)/J_mat1[g1]
                            comp_2          = -1 * G_1 * F1_2x[g1] #/sqrt(F1y**2+ F2y**2)
                            comp_2         += +1 * G_2 * F1_1x[g1] #/sqrt(F1y**2+ F2y**2)
                            # ...
                            G_1 = arr1_K11[g1]*( F1_2y[g1]*bi_x - F1_2x[g1]*bi_py)/J_mat1[g1] + arr1_K12[g1]*(-F1_1y[g1]*bi_x + F1_1x[g1]*bi_py)/J_mat1[g1]
                            G_2 = arr1_K12[g1]*( F1_2y[g1]*bi_x - F1_2x[g1]*bi_py)/J_mat1[g1] + arr1_K22[g1]*(-F1_1y[g1]*bi_x + F1_1x[g1]*bi_py)/J_mat1[g1]
                            comp_3          = -1 * G_1 * F1_2x[g1] #/sqrt(F1y**2+ F2y**2)
                            comp_3         += +1 * G_2 * F1_1x[g1] #/sqrt(F1y**2+ F2y**2)
                            # ...
                            G_1 = arr1_K11[g1]*( F1_2y[g1]*bj_x - F1_2x[g1]*bj_py)/J_mat1[g1] + arr1_K12[g1]*(-F1_1y[g1]*bj_x + F1_1x[g1]*bj_py)/J_mat1[g1]
                            G_2 = arr1_K12[g1]*( F1_2y[g1]*bj_x - F1_2x[g1]*bj_py)/J_mat1[g1] + arr1_K22[g1]*(-F1_1y[g1]*bj_x + F1_1x[g1]*bj_py)/J_mat1[g1]
                            comp_4          = -1 * G_1 * F1_2x[g1] #/sqrt(F1y**2+ F2y**2)
                            comp_4         += +1 * G_2 * F1_1x[g1] #/sqrt(F1y**2+ F2y**2)
                            #...
