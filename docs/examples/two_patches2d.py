...
#                                                    3
#------------------------------------------------------------------------------
def poisson_solve(V, pyrefMP, u11_mph, u12_mph, u21_mph, u22_mph, u_d1, u_d2):
...
# Initialize mapping
#------------------------------------------------------------------------------
xmp, ymp         = pyrefMP.getcoefs(1)   # First patch
wm1, wm2         = pyrefMP.weights()
xmp1, ymp1       = pyrefMP.getcoefs(2)   # Second patch
...
for ne in range(refGrid,refGrid+RefinNumber+1):
    ...
    weight, xmp, ymp  = pyrefMP.getRefineGeometryMap(num_patch=1, numElevate=nb_ne)
    wm1, wm2 = weight[:,0], weight[0,:]
    xmp1, ymp1  = pyrefMP.getRefineGeometryMap(num_patch=2, numElevate=nb_ne)[1:]
    # Create spline spaces for refined mesh
    V1 = SplineSpace(degree=degree[0], grid = pyrefMP.getRefinegrid(0, numElevate=nb_ne), nderiv = 1, omega = wm1, quad_degree = quad_degree)
    V2 = SplineSpace(degree=degree[1], grid = pyrefMP.getRefinegrid(1, numElevate=nb_ne), nderiv = 1, omega = wm2, quad_degree = quad_degree)
