...
#... utilities of anysotropic diffusion equation
#==============================================================================
...
def assemble_matrix_diff_ex01(ne1:'int', ne2:'int', p1:'int', p2:'int', p3:'int', p4:'int',
                            ...
                            vector_m1:'float64[:,:]', vector_m2:'float64[:,:]', dt:'float', matrix:'float64[:,:,:,:]'):
        ...
        for ie2 in range(0, ne2):
            ...
            for g1 in range(0, k1):
                for g2 in range(0, k2):
                    ...
                    c_per             = 0e0   #perpendicular coefficient
                    c_par             = 1e+8 #parallel coefficient
                    arr_K11[g1,g2]    =  c_per + (c_par - c_per) * bb11
                    arr_K12[g1,g2]    = (c_par - c_per) * bb12
                    arr_K22[g1,g2]    = c_per + (c_par - c_per) * bb22
                    # ...
                        ...
                        for jl_2 in range(0, p2+1):
                            ...
                            for g1 in range(0, k1):
                                for g2 in range(0, k2):
                                    ...
                                    Kbj_x = arr_K11[g1,g2] * bj_x + arr_K12[g1,g2] * bj_y 
                                    Kbj_y = arr_K12[g1,g2] * bj_x + arr_K22[g1,g2] * bj_y 

                                    wvol  = weights_1[ie1, g1] * weights_2[ie2, g2]

                                    v    += bi_0 * bj_0 * wvol * J_mat[g1,g2] + dt*(bi_x * Kbj_x + bi_y * Kbj_y ) * wvol / J_mat[g1,g2]
...
def assemble_vector_diff_ex01(ne1:'int', ne2:'int', p1:'int', p2:'int', p3:'int', p4:'int',
                            ...
                            basis_1:'float64[:,:,:,:]', basis_2:'float64[:,:,:,:]',  basis_3:'float64[:,:,:,:]', basis_4:'float64[:,:,:,:]',
                            weights_1:'float64[:,:]', weights_2:'float64[:,:]', points_1:'float64[:,:]', points_2:'float64[:,:]', 
                            vector_m1:'float64[:,:]', vector_m2:'float64[:,:]', vector_d:'float64[:,:]', vector_u:'float64[:,:]', ttime:'float', dt:'float', rhs:'float64[:,:]'):
    ...
    for ie1 in range(0, ne1):
        ...
        for ie2 in range(0, ne2):
            ...
            lcoeffs_sl[ : , : ] = vector_u[ i_span_1 : i_span_1+p1+1, i_span_2 : i_span_2+p2+1]
            for g1 in range(0, k1):
                for g2 in range(0, k2):
                    ...
                    # Laplacian of u = sqrt(S)
                    # lap_u = ( ( (6.0*x**2 + 2.0) / sqrt(4.0*y**2 + (x**2 - 1.0)**2)) - ((4.0*x**2*(x**2 - 1.0)**2 + 16.0*y**2) / (sqrt(4.0*y**2 + (x**2 - 1.0)**2)**3)) )
                    # # RHS
                    # f = -c_per * lap_u
                    # f = 0. #2.*pi**2*cos(pi*x)*cos(pi*y)
                    r2 = x**2 + y**2
                    a = 0.6
                    alpha = 0.01
                    
                    # manufactured solution
                    u = exp(ttime - ((r2 - a)**2)/alpha)
                    
                    # auxiliary quantities
                    coeff = 4*(r2 - a)/alpha
                    
                    # |grad h|^2
                    grad_h_sq = (coeff**2) * r2
                    
                    # Laplacian of h
                    lap_h = (8*(r2 - a)/alpha) + (16*r2/alpha)
                    
                    # Laplacian of u
                    lap_u = u * (grad_h_sq - lap_h)
                    
                    # forcing term
                    f = u - c_per * lap_u
                    lvalues_u[g1,g2]   = f * dt + usol
