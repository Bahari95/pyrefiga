"""
# anisotropic.py is a test file for the anisotropic diffusion

    -div(A . D(u)) = f  in \Omega
author: M Bahari
"""
...
class anisotropic_diffusion(object):
    ...
    def __init__(self, V, pyref_MP, x0, y0, dt=1e-6):
        ...
        for patch_nb in range(1, pyref_MP.nb_patches+1):
            ...
            stiffness  = assemble_matrix_diff(VT, fields=[u11_mph, u12_mph], value= [dt], out=stiffness)
    ...
    def diffusion(self, u_sol, dt, time = 0.):
        V                = self.V
        # ...
        ...
        for patch_nb in range(1, self.pyref_MP.nb_patches+1):
            ...
            # Assemble right-hand side vector
            rhs        = StencilVector(V.vector_space)
            rhs        = assemble_rhs_diff( VT, fields=[u11_mph, u12_mph, u_d1, u_sol[patch_nb-1]], value= [time, dt], out= rhs)
...
# ...
f_ex    = lambda x,y, t: 10.*np.exp(-100.*((x*cos(t)+y*sin(t)-x0)**2+(y*cos(t)-x*sin(t)-y0)**2))
g0      = lambda x,y : 10.*np.exp(-100.*((x-x0)**2+(y-y0)**2))
# ...
...
for nbRefine in range(nbRefineNbr+1):
    ...
    #------------------------------------------------------------------------------
    AN = anisotropic_diffusion(Vh, pyref_MP, x0, y0, dt= dt)
    # ...
    nt = 0
    # Project initial solution
    u_s, l2_error,  H1_error, mass_error = AN.project(g0)
    ...
    for i in range(pyref_MP.nb_patches):
        x_ap.append( pyccel_sol_field_2d((nbpts, nbpts), u_s[i].tensor,  Vh.knots, Vh.degree)[0])
    ...
    while (Ltime <time_max and nt <=Ntimes):
        ...
        u_s, l2_error,  H1_error, mass_error = AN.diffusion( u_s, dt, time = Ltime)
        #print('#')
        print(f"Step {nt}: Time = {Ltime:.4e} s | L2 Error = {l2_error:.4e} | H1 Error = {H1_error:.4e} | l1 mass = {mass_error:.4e}")
        # ...
        if nt%N_plot==0:
...
if args.plot:
    precomputed = [
        {"name": "app solution", "data": sol_app},
